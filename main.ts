...
const clampIndex = (i: number): number => Math.max(0, Math.min(SECTIONS.length - 1, i));
const angleFor = (i: number): number => SWEEP_START + i * SWEEP_STEP;
const pad = (i: number): string => `0${i}`.slice(-2);
